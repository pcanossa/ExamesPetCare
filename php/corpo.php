...
if (isset($_GET['pesquisa']) != 'sim') {
        ...
        $select="SELECT animal, tutor, especie, exame, qtdade, realizador, datareal, estatus, envio, cod FROM exames where estatus='Em Realização'";
        $query=mysqli_query($con, $select);
        $rows=mysqli_num_rows($query);
        echo "<div class='cabcorpo'><h3>Exames Aguardando Execução</h3></div>";
        echo "<div class='container cabexecut'><p>Animal</p><p>Tutor</p><p>Data de Solicitação</p><p>Exame </p><p>Realizador</p><p>Status de Envio</p></div><br>";
        $count=0;
        ...
        echo "<form method='post' class='emexecut'><a class='container resultpesq' href='php/recebeDados.php?cod=$cod'><p>$animal</p><p>$tutor</p><p>$dia/$mes/$ano</p><p>$exame</p><p>$realizador</p><p class='enviorel' id='enviorel$count'>$envio</p></a></form><br>";
        $count++;
        ...
        <script src=\"/ExamesPetCare/javascript/enviobusca.js\";>
        </script>";      
} else { 
        session_start();
...
if ($validador=='true') {
    ...
    if ($rows>0) {
        while ($result = mysqli_fetch_array($query)){
            ...
            echo "<form  class='emexecut' method='post'><a class='container resultpesq' href='php/recebeDados.php?cod=$cod'><p>$dia/$mes/$ano </p><p>$animal </p><p>$tutor</p><p>$exame </p><p>$realizador </p><p class='estat' id='str$count'>$estatus </p></a></form><br>";
            $count++;
        }
